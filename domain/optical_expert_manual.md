...
## 8. 复杂结构与多镜组/模块化系统设计工程规程 (Multi-Group & Modular Optical Engineering SOP)

在现代激光扫描显微镜（共聚焦/双光子）、高精度光学中继（Relay）、内窥镜、微纳投影以及航测遥感等复杂系统中，光路均由**多个相对独立但紧密协作的子光学系统（如：扫描镜 + 中间像面 + 筒镜 + 显微物镜）**串联构成。此类系统绝不能当作单镜头进行一锅端盲目优化，必须执行严格的模块化工程 SOP。

### 8.1 三大核心接口解耦契约 (The 3 Decoupling Contracts)

1. **光瞳共轭与放大率契约 (Pupil Conjugation Contract)**：
   - 振镜偏转转轴中心（系统物理光阑面）必须**严格光学共轭**于物镜后焦面（BFP / 入瞳面）；
   - 根据拉格朗日不变量，光瞳放大率严格满足：
     $$M_{\text{pupil}} = \frac{f_{\text{tube}}}{f_{\text{scan}}} = \frac{D_{\text{BFP}}}{D_{\text{galvo}}}$$
   - **工程红线**：光瞳若发生轴向漂移，振镜扫描时光束将在物镜后焦面发生横向走位（Pupil Walking），导致边缘视场发生高达 40%~60% 的严重几何渐晕（Vignetting / Shading）。
2. **中间像面与双远心契约 (Intermediate Image & Double Telecentricity Contract)**：
   - **扫描透镜像方远心**：在全扫描视场范围内，扫描透镜聚焦在中间像面的主光线入射角（CRA）必须 $\le 0.5^\circ$；
   - **筒镜物方远心**：筒镜必须以平行的主光线姿态（CRA $\le 0.5^\circ$）接收中间像面的发散光束；
   - **中间像面自洽独立性**：中间像面本身必须是独立的平场（Petzval 消除）且接近衍射极限（RMS 波前像差 $\le 0.04\lambda$），**严禁在中间像面遗留巨大畸变/场曲并指望后组反向抵消！**
3. **无限远准直空间契约 (Infinity Space Contract)**：
   - 筒镜出射进入物镜的光束必须是**绝对平行准直光（倾角 $\theta \le 0.001^\circ$）**；
   - 保证在该空间内自由插入二色镜、滤光片时，不引入任何附加球差与离焦。

### 8.2 工业界标准五阶段闭环工作流 (Standard 5-Stage Modular Workflow)

```mermaid
graph TD
    A["Stage 1: 一阶高斯拓扑与拉格朗日不变量切分 (Paraxial Layout)"] 
    --> B["Stage 2: 像差与公差预算切分 (RSS 方和根准则)"]
    B --> C1["Stage 3A: 扫描透镜离线自洽独立设计"]
    B --> C2["Stage 3B: 筒镜离线自洽独立反向设计"]
    C1 --> D["Stage 4: 理想近轴透镜 (Paraxial Lens) 隔离替代测试"]
    C2 --> D
    D --> E["Stage 5: 阶梯式四步联调释放法 (全锁定 -> 仅调空气距 -> 局部阻尼微调 -> 波前平衡)"]
```

1. **Stage 1 (一阶规划)**：严禁盲目插透镜，先用 Python/Excel 算定 $f_{\text{obj}}$、$D_{\text{BFP}}$、$D_{\text{galvo}}$、$M_{\text{pupil}}$，锁定 $f_{\text{scan}}$ 与 $f_{\text{tube}}$。
2. **Stage 2 (预算切分)**：全系统波前像差 $\le 0.070\lambda$ 分配为：物镜 $\le 0.045\lambda$、扫描镜 $\le 0.035\lambda$、筒镜 $\le 0.030\lambda$、装配对准 $\le 0.030\lambda$。
3. **Stage 3 (离线自洽)**：扫描透镜、筒镜、物镜在各自独立文件里设计并达到衍射极限，中间像面与光瞳均达标。
4. **Stage 4 (近轴替代测试)**：联调前用理想近轴透镜（Paraxial Surface）替换其他模块，100% 隔离像差来源，精准定位单一模块残差。
5. **Stage 5 (阶梯式联调)**：全透镜表面 100% 锁定，仅释放组间机械空气距以消除共轭离焦，随后限制曲率浮动 $\pm 5\%$ 阻尼微调，最后锁定波前。

### 8.3 跨模块像差防代偿红线与评价函数“铁幕硬屏障” (The Operand Barrier Matrix)

- **物理机理**：严禁后组拿负球差抵消前组正球差！轴上看似为零，但离轴光束投影错位会激发出**巨额的三阶彗差（Coma）**，且使温漂与轴向公差敏感度暴增数千倍。
- **评价函数硬屏障矩阵清单**：
  - `EFLA Surf_start Surf_end`：分段锁死扫描透镜和筒镜的独立有效焦距（Target = 标称值，Weight $\ge 100$）；
  ...
  - `MXCA`：**强制单段内部空气间隙上限 $\le 8.0 \sim 12.0\,\text{mm}$（权重 500）**，彻底扼杀优化器通过拉长轴向空气间隙偷懒消除场曲的逃逸企图；
  - `MNEG`：**强制玻璃边缘厚度 $ET \ge 1.2 \sim 1.5\,\text{mm}$（权重 500）**，严禁刀口尖角与胶合面矢高崩穿（Sag Crossover）；
  - `MNEA`：**强制边缘空气净空 $\ge 0.8 \sim 1.0\,\text{mm}$（权重 200）**，预留机械隔圈支撑台阶。

### 8.4 DFM 面向制造与机械装配纪律 (DFM & Drop-in Assembly)

1. **单镜筒深径比**：单段镜筒深径比严格控制在 $L/D \le 2.0 \sim 2.5 : 1$。总长超标时必须分段独立制造，采用精密定位止口法兰（配合间隙 $< 5\,\mu\text{m}$）螺栓对接。
...
3. **[ ] 内部单段空气间隔限制**：除外置后截距与前置工作距外，任意单段镜片间空气中心厚度是否严格满足 $t_c \le 12.0\text{ mm}$（极限 $\le 15.0\text{ mm}$）？
4. **[ ] 镜组叠层总厚度比**：镜组玻璃与空气叠层总厚度是否满足 $L_{\text{barrel}} \le 0.35 \sim 0.6 \times \text{EFL}$？
5. **[ ] 镜筒深径比限制**：单段镜筒有效装配深度与内径之比是否 $\le 2.5:1$？
6. **[ ] 边缘厚度防刀口**：所有透镜边缘厚度是否均满足 $ET \ge 1.2\text{ mm}$（塑料模压 $\ge 0.8\text{ mm}$）？
7. **[ ] 边缘碰撞净空裕量**：所有相邻透镜在全视场边缘处是否有 $t_{\text{air-edge}} \ge 0.8\text{ mm}$ 的物理间隙，防止边缘相碰？
8. **[ ] 玻璃中心厚径比**：所有玻璃元件是否满足 $CT \ge 1.0\text{ mm}$ 且 $CT / D \ge 0.08$？
9. **[ ] 检验样板陡度比**：所有球面曲率半径是否满足 $|R| \ge 1.2 \times \text{Semi-Diameter}$，无无法测量的深凹面？
10. **[ ] 玻璃选型可制造性**：是否全部采用 CDGM 或 Schott 常用优先推荐目录玻璃？外露面是否避开了耐酸级差的软玻璃？
11. **[ ] 非球面高阶项截断**：非球面是否仅放开 4 阶和 6 阶？是否排除了 8 阶以上系数导致的波纹拐点？
12. **[ ] 偏折角公差钝化**：单表面主光线与边缘光线最大入射角是否控制在 $\le 35^\circ \sim 45^\circ$ 以内？
